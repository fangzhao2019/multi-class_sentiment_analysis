        ...
        if target_labels is not None:
            for t in range(mcn):
                if t > 0:
                    ...
                    dec_mask = dec_labels > 0.5
                state = self._decoder_step(state, dec_labels, dec_mask, clause_num_mask, label_embed, t)
            ...
            for t in range(mcn):
                if t > 0:
                    ...
                    dec_mask = (torch.sigmoid(pred_scores) > 0.5).float()
                state = self._decoder_step(state, dec_labels, dec_mask, clause_num_mask, label_embed, t)
                pred_scores = self.output_layer(state["decoder_hidden"])
                pred_sigmoid_scores = torch.sigmoid(pred_scores)
                step_pred_labels[:, t, :] = pred_sigmoid_scores
        return step_pred_labels

    def _decoder_step(self, state, last_predictions, dec_mask, clause_num_mask, label_embed, t):
        bz, mcn, hs = state['clause_vector'].size()

        label_weight = masked_softmax(last_predictions, dec_mask).unsqueeze(1)
        embedded_input = torch.bmm(label_weight, label_embed).view(bz, -1)
