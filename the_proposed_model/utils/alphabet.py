...
class Alphabet:
    #词表与id的对应操作
    #name主要是区分不同的alphabet
    #lable是控制是否需要unknown
    #keep_growing控制新词是否加入词表
    ...
    def add(self, instance): # 将新词添加到现有的alphbet
        if instance not in self.instance2index:
            self.instances.append(instance)
            self.instance2index[instance] = self.next_index
            self.next_index += 1

    def get_index(self, instance): # 返回指定词的index
        try:
    ...
    def get_instance(self, index): # 给定index返回词
        #if index == 0:
