...
class CNN_BiLSTM(nn.Module):
    ...
    def forward(self, x, batch_clauselen):
        ...
        x2=x2.view(self.batch_size, self.sentenceNumber, self.out_channels)
        ...
        x3=x3.view(self.batch_size, self.sentenceNumber, self.out_channels)

        x4=F.relu(self.conv4(x))
        x4=self.batchNorm4(x4)
        x4=x4.view(self.batch_size*self.sentenceNumber,self.out_channels,self.conv_dim4)
        x4=F.max_pool2d(x4,(1,self.conv_dim4))
        x4=x4.view(self.batch_size, self.sentenceNumber, self.out_channels)

        x=torch.cat((x2,x3,x4),2)
        #print(x.size())
